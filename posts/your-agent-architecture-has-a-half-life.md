...
<div class="video">
    <iframe
        src="https://www.youtube.com/embed/X1kp-ABIIxQ"
        frameborder="0"
        allowfullscreen
    ></iframe>
</div>
...
I've written before about [why agents need a harness, not a framework](https://www.inngest.com/blog/your-agent-needs-a-harness-not-a-framework). The harness is the infrastructure your agent sits on. Not the agent logic itself, not your product. The scaffolding that connects, protects, and orchestrates the components. Here's the deeper model.
...
Your prompts last weeks if you're lucky. A new model drops, capabilities shift, and suddenly that [carefully tuned chain-of-thought prompt](https://www.inngest.com/blog/context-engineering-is-software-engineering-for-llms) needs a rewrite. Your tools change when you add a new integration or swap an API. Your RAG pipeline changes when you restructure your data or switch embedding models. Context has a half-life measured in weeks.
...
![Execution layer overview in relations to the context and compute layers](/images/posts/your-agent-architecture-has-a-half-life/execution-loop.png)
...
Agents handle longer-running tasks than ever. [Background agents](https://www.inngest.com/blog/background-agents-are-here-your-orchestration-isnt-ready) run for minutes to hours. A three-hour run can't hold state in memory or on disk. A failed API call 38 steps into [a loop](https://www.inngest.com/docs/ai-patterns/agent-tool-loops) should mean retry step 38, not restart from step 1.
...
Agents need flexible invocation. Event-driven. Cron-scheduled. API-triggered. [Human-in-the-loop approvals](https://www.inngest.com/docs/ai-patterns/human-in-the-loop). And they need to coordinate with other agents, both synchronously and asynchronously.
...
Flexible execution primitives let you build any pattern your harness needs. Sync delegation with `step.invoke`. Async fan-out with `step.sendEvent`. Human-in-the-loop with `step.waitForEvent`. These are the building blocks. The [three sub-agent patterns](https://www.inngest.com/blog/three-patterns-you-need-for-agentic-systems) you need for agentic systems all reduce to combinations of these primitives.
...
![Execution layer trace with all spans across your run](/images/posts/your-agent-architecture-has-a-half-life/trace.png)
...
You can't build these without durable execution. I wrote about this in detail in [Background Agents Are Here](https://www.inngest.com/blog/background-agents-are-here-your-orchestration-isnt-ready). The short version: if your orchestration can't survive a single failure in a three-hour run, you don't have orchestration. You don't have confidence either.
...
![Loop architecture diagram with cron agent, sub-agents, and review agent](/images/posts/your-agent-architecture-has-a-half-life/loop.png)
...
For the deep dive on loop architectures with full code examples, check out [The Agent Loop Architecture](https://www.inngest.com/blog/agent-loop-architecture).
...
Most teams score model outputs. That's useful but incomplete. [Score on product outcomes](https://www.inngest.com/blog/introducing-agent-evals).
...
Don't just sample a few runs. [Score everything your agent does](https://www.inngest.com/blog/introducing-scoring). `step.waitForEvent` lets you wait for real user behavior: did they save the result? Did they edit it? Did they discard it? That's signal you can't get from evaluating model outputs in isolation.
...
![Execution layer as the hub for all data around agent architectures including traces, user feedback, scores, and insights](/images/posts/your-agent-architecture-has-a-half-life/evals.png)
...
[Inngest](https://www.inngest.com/) is the execution layer of the harness. The durable coordination layer that sits on top of both and outlasts them.
...
The entire post above is the argument for why this layer matters. [Check out the docs](https://www.inngest.com/docs) and start building.
