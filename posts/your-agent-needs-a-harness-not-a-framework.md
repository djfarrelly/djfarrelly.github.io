...
![Utah harness architecture](/images/posts/your-agent-needs-a-harness-not-a-framework/utah-harness-architecture.png)
...
For a step-by-step walkthrough of this pattern, see the [agent tool loop guide](https://www.inngest.com/docs/ai-patterns/agent-tool-loops).
...
![Utah harness functions](/images/posts/your-agent-needs-a-harness-not-a-framework/utah-harness-functions.png)
...
To make it multi-player, we're going to explore [swappable sandboxes, external state and memory](https://www.inngest.com/blog/your-agent-architecture-has-a-half-life). This would enable Utah to run on serverless if someone wanted to.

There are a lot of fun UX things we'd like to add built on the Inngest API and our Insights feature to build session monitoring for coding sessions. We will also explore using `step.waitForEvent()` to create [human-in-the-loop approval flows](https://www.inngest.com/docs/ai-patterns/human-in-the-loop) when more input is needed.

The last piece that we're exploring to make this truly “universally triggered” is [enabling Utah to write itself](https://www.inngest.com/blog/agent-loop-architecture) — building new agents and workflows, creating new webhooks, and monitoring itself via API. If you're interested, share some ideas on the GitHub repo.
