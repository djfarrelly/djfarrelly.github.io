...
But durability alone isn't enough. Reliability through durable execution requires **[flow control](https://www.inngest.com/platform/flow-control)**. Flow control is everything listed above that you have to build out for reliable workflows around queues. While durable execution focuses on guaranteeing the execution of tasks and handling failures, flow control deals with managing how or when the code is executed.
...
With durable execution, all this can be encapsulated in a single workflow function like the one above.
