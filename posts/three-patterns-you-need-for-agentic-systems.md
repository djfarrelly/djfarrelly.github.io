...
The trends of sub-agents has shifted from specialized to generic, but regardless of your chosen approach, each system ([or harness if you will](https://www.inngest.com/blog/your-agent-needs-a-harness-not-a-framework)), needs to have three core patterns for working with sub-agents.
...
![Sub-agent architecture diagram](/images/posts/three-patterns-you-need-for-agentic-systems/utah-sub-agents.png)

Every agentic system needs all three and they all have their own purpose and considerations. For implementation details, see the [sub-agent delegation guide](https://www.inngest.com/docs/ai-patterns/sub-agent-delegation). Let's dive into each of them.
...
- [**Self-iterating agents**](https://www.inngest.com/blog/agent-loop-architecture). With scheduled sub-agents, why couldn't the agents keep iterating on themselves and the systems. Cost and budgets will naturally come into consideration.
